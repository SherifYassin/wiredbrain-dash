...
export class RegisterPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams,
    public alerCtrl: AlertController, public afAuth: AngularFireAuth) {
  }
  ...
  registerAccount(){
    if(this.reg.passWrd1 != this.reg.passWrd2){
      this.displayAlert("Password Problem", "Passwords does not match. Please try again");
      this.reg.passWrd1 = "";
      this.reg.passWrd2 = "";
    }
    else{
  ...
  regSuccess(result){
    this.displayAlert(result.email, "An account was created for this email address");
